...
class Printer {
    ...
    static void printJsonRequest(LoggingInterceptor.Builder builder, Request request) throws UnsupportedEncodingException {
        ...
        if (request.body() instanceof FormBody) {
            ...
            if (body != null && body.size() != 0) {
                for (int i = 0; i < body.size(); i++) {
                    formBody.append(body.encodedName(i) + "=" + URLDecoder.decode(body.encodedValue(i), "UTF-8") + "&");
                }
    ...
    static void printFileRequest(LoggingInterceptor.Builder builder, Request request) throws UnsupportedEncodingException {
        String tag = builder.getTag(true);
        if (builder.getLogger() == null)
            I.log(builder.getType(), tag, REQUEST_UP_LINE);
        logLines(builder.getType(), tag, new String[]{URL_TAG + request.url()}, builder.getLogger(), false);
