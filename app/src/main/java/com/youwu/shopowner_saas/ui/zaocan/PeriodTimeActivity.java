 ...
 * @date: 2023/04/24
 */
public class PeriodTimeActivity extends BaseActivity<ActivityPeriodTimeBinding, PeriodTimeViewModel> {


    String store_id;//门店id
    String channel_id;//1早餐/2食材
    String appointment_time;//配送时间
    private SmartTable<PeriodInfo> periodTable;

    List<PeriodInfo> list = new ArrayList<>();
    String TopName;
    ...
    public void initViewObservable() {
        ...
        viewModel.PeriodInfoEvent.observe(this, new Observer<ArrayList<PeriodInfo>>() {
            @Override
            public void onChanged(ArrayList<PeriodInfo> periodInfos) {
                list.clear();
                for(int i = 0;i <periodInfos.size(); i++) {

                    list.add(new PeriodInfo(periodInfos.get(i).getTime(), "  "+periodInfos.get(i).getGoodsName(),periodInfos.get(i).getNumber()));
    ...
    //查询
    private void getTimeGoodsInfo() {
        appointment_time= TimeUtil.getTimeStemp(viewModel.TimeField.get(),"yyyy-MM-dd")/1000+"";
        viewModel.getTimeGoodsInfo(channel_id,store_id,appointment_time);
    }
    ...
    public void initData() {
        ...
        periodTable = (SmartTable<PeriodInfo>) findViewById(R.id.periodTable);
