...
public class OrderGoodsRecycleAdapter extends RecyclerView.Adapter<OrderGoodsRecycleAdapter.myViewHodler> {
    private Context context;
    private List<OrderDetailsBean.GoodsListBean> goodsEntityList;
    private int currentIndex = 0;

    //创建构造函数
    public OrderGoodsRecycleAdapter(Context context, List<OrderDetailsBean.GoodsListBean> goodsEntityList) {
        //将传递过来的数据，赋值给本地变量
    ...
    public void onBindViewHolder(final myViewHodler holder, @SuppressLint("RecyclerView") final int position) {
        ...
        //根据点击位置绑定数据
        final OrderDetailsBean.GoodsListBean data = goodsEntityList.get(position);
        ...
        holder.tv_number.setText("X "+data.getGoods_number());

        Glide.with(context).load(data.getGoods_thumb()).placeholder(R.mipmap.loading).into(holder.goods_image);
    ...
    public interface OnItemClickListener {
         ...
         */
        public void OnItemClick(View view, OrderDetailsBean.GoodsListBean data, int position);
    }
