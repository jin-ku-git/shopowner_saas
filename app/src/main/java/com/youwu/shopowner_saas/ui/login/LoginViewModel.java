...
public class LoginViewModel extends BaseViewModel<DemoRepository> {
    ...
    //1 收银员 2、加盟商
    public ObservableField<Integer> login_type = new ObservableField<>(0);
    //用户名清除按钮的显示隐藏绑定
    ...
    //切换登录
    public BindingCommand SYYOnClickCommand = new BindingCommand(new BindingAction() {
        @Override
        public void call() {
            login_type.set(1);
        }
    ...
    //切换登录
    public BindingCommand JMSOnClickCommand = new BindingCommand(new BindingAction() {
        @Override
        public void call() {
            login_type.set(2);
        }
    ...
    private void login() {
        ...
        model.LOGIN(userName.get(),password.get(),login_type.get())
                ...
                .subscribe(new DisposableObserver<BaseBean<Object>>() {
                    ...
                    public void onNext(BaseBean response) {
                        ...
                        if (response.isOk()){
                            ...
                            LogBean logBean = JSON.parseObject(toPrettyFormat(JsonData), LogBean.class);
                            UserUtils.setToken(logBean.getAccessToken());
                            UserUtils.setTokenType(logBean.getTokenType());
                            //保存最新的登录时间
                            SimpleDateFormat formater = new SimpleDateFormat("yyyy.MM.dd HH:mm:ss");
                            String time= formater.format(new Date());
                            UserUtils.setLogoTime(time);
                            UserUtils.setLogoName(logBean.getUserInfo().getUserName());
                            //跳转到首页
